...
def extract_positive_id(result: Any) -> str:
    """Return a strictly positive Bitrix ID or fail closed."""
    value = result
    ...
    try:
        ...
        raise RuntimeError(f"user.add не вернул числовой ID: {result!r}") from exc
    if number <= 0:
        raise RuntimeError(f"user.add вернул некорректный ID: {result!r}")
    return str(number)


def _report_row(action: Action, include_sensitive: bool) -> dict[str, Any]:
    row = asdict(action)
    if not include_sensitive:
        row["full_name"] = "[REDACTED]" if row.get("full_name") else ""
        row["email"] = "[REDACTED]" if row.get("email") else ""
    for key, value in list(row.items()):
        if isinstance(value, str):
            row[key] = excel_literal(value, pii=not include_sensitive)
    return row


def write_reports(actions: list[Action], mode: str, source_file: str) -> None:
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    include_sensitive = str(os.environ.get("B24_REPORT_SENSITIVE_PAYLOADS", "")).strip().lower() in {
        "1", "true", "yes", "on"
    }
    summary = {
        "mode": mode,
        "source_file": Path(source_file).name,
        "sensitive_rows_included": include_sensitive,
    ...
    rows = [_report_row(action, include_sensitive) for action in actions]
    ...
    error_rows = [
        _report_row(action, include_sensitive)
        for action in actions
...
def main() -> int:
    ...
    parser.add_argument("--file", default="input/users_to_invite.xlsx", help="Путь к Excel-файлу")
    ...
    for row in input_users:
        ...
        if args.mode == "dry_run":
            ...
            # Reserve the identity inside this run so a second row with the same
            # FIO and another email is not planned as a second account.
            planned_user = {
                "ID": "",
                "EMAIL": row.email.strip(),
                "NAME": row.name.strip(),
            ...
            users_by_email[email] = planned_user
            users_by_name.setdefault(normalize(row.full_name), []).append(planned_user)
            continue

        try:
            result = client.call("user.add", fields)
            target_id = extract_positive_id(result)
            created_user = {
                "ID": target_id,
                "EMAIL": row.email.strip(),
                "NAME": row.name.strip(),
            ...
            users_by_name.setdefault(normalize(row.full_name), []).append(created_user)
            actions.append(
                Action(
                    **action_base,
                    status="OK",
            ...
            actions.append(
                Action(
                    **action_base,
                    status="ERROR",
                    message=sanitize_secret_text(str(exc), pii=True),
                )
            )
    ...
    for action in actions:
        safe_message = sanitize_secret_text(action.message, pii=True)
        print(
            f"{action.status:7} | row {action.row_number:>3} | "
            f"{action.department} ({action.department_id}) | {safe_message}"
        )
