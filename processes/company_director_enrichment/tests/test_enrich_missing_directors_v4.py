...
def _row(company_id: int, owner_id: int = 72) -> dict:
    return {"company_id": company_id, "title": f"Компания {company_id}", "owner_id": owner_id, "bin": f"{company_id:012d}", "requisite_ids": [company_id + 100], "status": "accepted", "director": DIRECTOR, "source": "adata", "url": "https://adata/example", "confidence": "single_source", "director_group_size": 1}

class PlanClient:
    def __init__(self, *, canonical=None, company_owners=None, lead_contacts=None, contact_companies=None, lead_owners=None):
        self.canonical = canonical
        self.company_owners = company_owners or {}
        self.lead_contacts = lead_contacts or {}
        self.contact_companies = contact_companies or {}
        self.lead_owners = lead_owners or {}
        self.calls = []
    ...
    def call(self, method, payload=None):
        ...
        if method == "crm.company.get":
            ...
            return {"ID": str(company_id), "ASSIGNED_BY_ID": str(self.company_owners[company_id]), "ORIGIN_ID": ""}
        if method == "crm.company.contact.items.get":
            return []
        if method == "crm.contact.company.items.get":
            return [{"COMPANY_ID": company_id, "IS_PRIMARY": "Y" if idx == 0 else "N"} for idx, company_id in enumerate(self.contact_companies.get(int(payload["id"]), []))]
        if method == "crm.lead.contact.items.get":
            lead_id = int(payload["id"])
            return [{"CONTACT_ID": contact_id, "IS_PRIMARY": "Y" if idx == 0 else "N"} for idx, contact_id in enumerate(self.lead_contacts.get(lead_id, []))]
    ...
    def list_all(self, method, payload=None):
        ...
        if method == "crm.requisite.list":
            ...
            return [{"ID": str(company_id + 100), "ENTITY_ID": str(company_id), "RQ_INN": f"{company_id:012d}", "RQ_DIRECTOR": ""}]
        if method == "crm.lead.list":
            company_id = int(payload["filter"]["COMPANY_ID"])
            lead_ids = [company_id * 100 + 1, company_id * 100 + 2]
            return [{"ID": str(lead_id), "ASSIGNED_BY_ID": str(self.lead_owners.get(lead_id, self.company_owners[company_id]))} for lead_id in lead_ids]
        raise AssertionError(method)

def test_plan_reuses_existing_contact_and_exposes_future_31_reassignment(monkeypatch):
    client = PlanClient(canonical={"ID": "50", "LAST_NAME": "Оспанова", "NAME": "Назым", "SECOND_NAME": "Батырбековна", "POST": "Руководитель", "COMMENTS": "", "ASSIGNED_BY_ID": "58"}, company_owners={10: 72}, lead_contacts={1001: [50], 1002: [99]}, contact_companies={50: [99]}, lead_owners={1001: 58, 1002: 72})
    monkeypatch.setattr(v4.base, "clone_client", lambda client: client)
    row = v4.build_dry_run_plan(client, [_row(10)], workers=1)[0]
    ...
    assert row["workflow31_company_would_reassign"] == "YES"
    assert row["workflow31_would_reassign"] == "YES"
    assert row["workflow31_target_owner_id"] == 58
    assert row["lead_owner_snapshot"] == "1001:58,1002:72"
    assert row["workflow31_leads_to_reassign"] == "1002"
    assert row["workflow31_leads_to_reassign_count"] == 1
...
def test_plan_exposes_lead_owner_reassignment_even_when_company_owner_matches(monkeypatch):
    client = PlanClient(canonical={"ID": "50", "LAST_NAME": "Оспанова", "NAME": "Назым", "SECOND_NAME": "Батырбековна", "POST": "Руководитель", "COMMENTS": "", "ASSIGNED_BY_ID": "72"}, company_owners={10: 72}, lead_contacts={1001: [50], 1002: [50]}, contact_companies={50: [10]}, lead_owners={1001: 72, 1002: 58})
    monkeypatch.setattr(v4.base, "clone_client", lambda client: client)
    row = v4.build_dry_run_plan(client, [_row(10)], workers=1)[0]
    assert row["owner_mismatch"] == "NO"
    assert row["workflow31_company_would_reassign"] == "NO"
    assert row["workflow31_would_reassign"] == "YES"
    assert row["workflow31_leads_to_reassign"] == "1002"
    assert row["workflow31_leads_to_reassign_count"] == 1
...
def test_plan_is_read_only(monkeypatch):
    ...
    mutation_methods = [method for method, _payload in client.calls if method.endswith(".add") or method.endswith(".update") or method.endswith(".delete")]
    assert mutation_methods == []

def test_v4_workbook_has_plan_columns_and_no_business_analyst_columns(tmp_path):
    row = _row(10)
    row.update({"plan_status": "READY", "planned_contact_action": "CREATE", "planned_contact_id": "NEW", "planned_contact_owner_id": 72, "company_owner_id": 72, "owner_mismatch": "NO", "workflow31_company_would_reassign": "NO", "workflow31_would_reassign": "NO", "workflow31_target_owner_id": 72, "lead_owner_snapshot": "1001:72,1002:72", "workflow31_leads_to_reassign": "", "workflow31_leads_to_reassign_count": 0, "director_group_company_ids": "10", "companies_to_link": "10", "company_link_action": "CREATE_PRIMARY", "requisites_to_update": "110", "requisites_to_update_count": 1, "lead_ids_all": "1001,1002", "leads_to_link": "1001,1002", "leads_to_link_count": 2, "existing_lead_links": "", "existing_lead_links_count": 0})
    ...
    assert "31 что-то переназначит" in shared
