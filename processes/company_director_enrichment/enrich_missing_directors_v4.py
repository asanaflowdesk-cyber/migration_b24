...
def _safe_lead_plan(
    ...
    to_link: list[int] = []
    existing: list[int] = []
    for lead in v2._company_leads(client, company_id):
        ...
        if existing_contact_id in linked_ids:
            ...
            to_link.append(lead_id)
    return all_leads, to_link, existing
...
def _plan_group(client: BitrixClient, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    if canonical:
        if existing_contact_id is None:
            ...
            # Mirrors apply: if an existing contact has no owner, owner can only
            # be inferred when all candidate companies point to one owner.
            candidate_owners = {
    ...
    if existing_contact_id is not None:
        existing_contact_company_ids = v2._binding_ids(
            v2._contact_company_bindings(client, existing_contact_id),
            "COMPANY_ID",
        )

    group_companies_to_link = [
        company_id for company_id in group_company_ids
        if existing_contact_id is None or company_id not in existing_contact_company_ids
    ]
    ...
    for row in ordered:
        ...
        result.update({
            ...
            "plan_block_reason": "",
            "planned_contact_action": planned_contact_action,
            ...
            "companies_to_link": _csv(group_companies_to_link),
        ...
        result["workflow31_would_reassign"] = "YES" if mismatch else "NO"

        if existing_contact_id is not None:
            result["company_link_action"] = (
                "ALREADY_LINKED" if company_id in existing_contact_company_ids else "ADD_LINK"
            )
        else:
            result["company_link_action"] = (
                "CREATE_PRIMARY" if company_id == primary_new_company_id else "ADD_SECONDARY_AFTER_CREATE"
            )
        ...
        all_leads, to_link, existing = _safe_lead_plan(client, company_id, existing_contact_id)
...
def build_dry_run_plan(
    ...
    """Build a read-only execution plan mirroring apply without CRM writes."""
    untouched = []
    ...
    with ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix="director-plan") as pool:
        futures = {
            pool.submit(_plan_group, base.clone_client(client), group_rows): key
            for key, group_rows in accepted_groups.items()
        }
        done = 0
...
def _write_workbook_v4(output_dir: Path, rows: list[dict[str, Any]], skipped: list[dict[str, Any]]) -> None:
    path = output_dir / "company_director_enrichment.xlsx"
    ...
    columns = [
        ...
        ("owner_mismatch", "Ответственные расходятся"),
        ("workflow31_would_reassign", "31 потом сменит ответственного"),
        ...
        ("duplicate_contact_ids", "Дубли контакта"),
    ...
    widths = {
        0: 11, 1: 38, 2: 16, 3: 34, 4: 11, 5: 15, 6: 20, 7: 14, 8: 30,
        9: 18, 10: 16, 11: 20, 12: 16, 13: 20, 14: 20, 15: 18, 16: 22, 17: 12,
        18: 12, 19: 24, 20: 24, 21: 24, 22: 24, 23: 24, 24: 14, 25: 28, 26: 28,
        27: 14, 28: 28, 29: 14, 30: 24, 31: 18, 32: 34, 33: 45, 34: 18, 35: 34,
        36: 45, 37: 28, 38: 45, 39: 28,
    }
    for col, width in widths.items():
        sheet.set_column(col, col, width)
    sheet.set_column(40, len(columns) - 1, 22)
...
def write_report_v4(
    ...
    create_groups = {
        ...
        if row.get("planned_contact_action") == "CREATE" and base.valid_fio(row.get("director"))
    }
    reuse_groups = {
        base.fio_key(row.get("director"))
        for row in ready_rows
        if row.get("planned_contact_action") == "REUSE" and base.valid_fio(row.get("director"))
    }
    ...
    summary.update({
        ...
        "plan_owner_mismatches": sum(row.get("owner_mismatch") == "YES" for row in ready_rows),
        "plan_company_links_to_add": sum(
            row.get("company_link_action") in {"CREATE_PRIMARY", "ADD_SECONDARY_AFTER_CREATE", "ADD_LINK"}
            for row in ready_rows
        ),
        "plan_requisites_to_update": sum(int(row.get("requisites_to_update_count") or 0) for row in ready_rows),
        "plan_lead_links_to_add": sum(int(row.get("leads_to_link_count") or 0) for row in ready_rows),
        "plan_lead_links_existing": sum(int(row.get("existing_lead_links_count") or 0) for row in ready_rows),
        "plan_duplicate_contacts_detected": len(duplicate_ids),
        "plan_multi_company_director_groups": len({
            base.fio_key(row.get("director"))
            for row in ready_rows
            if int(row.get("director_group_size") or 0) > 1 and base.valid_fio(row.get("director"))
        }),
    })
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Enrich missing company directors from Adata OR Kompra and produce "
            "a read-only execution plan before applying canonical contact/company/lead links"
        )
    )
    ...
    if args.apply:
        # Apply only rows that passed the read-only plan. Blocked rows are kept
        # in the report untouched and are never sent into mutating CRM calls.
        ready_rows = [row for row in rows if row.get("plan_status") == PLAN_READY]
    ...
    if args.apply and summary.get("plan_blocked"):
        ...
        (Path(args.output_dir) / "company_director_enrichment_summary.json").write_text(
            json.dumps(summary, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
