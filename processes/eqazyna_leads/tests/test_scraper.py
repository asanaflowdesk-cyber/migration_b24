...
def test_build_url_supports_exact_document_number_without_status_filter():
    scraper = EqazynaScraper(polite_delay_seconds=0)
    url = scraper.build_url(
        1,
        "Заявка на разведку ТПИ",
        [],
        doc_number="49701-NEA",
    )

    assert "flDocNum=49701-NEA" in url
    assert "flStatus=" not in url
    assert "flDocType=%D0%A2%D0%BF%D0%B8%D0%97%D0%B0%D1%8F%D0%B2%D0%BA%D0%B0%D0%9D%D0%B0%D0%A0%D0%B0%D0%B7%D0%B2%D0%B5%D0%B4%D0%BA%D1%83" in url


class _ExactLookupScraper(EqazynaScraper):
    def __init__(self):
        super().__init__(polite_delay_seconds=0)
        self.lookup = None

    def fetch_page(self, page, doc_type, statuses, doc_number=None):
        self.lookup = (page, doc_type, tuple(statuses), doc_number)
        return _registry_html_with_status(
            [
                ("49700-NEA", "000000000001", "Принято"),
                ("49701-NEA", "000000000002", "Выдана лицензия"),
            ]
        ), "https://example.test/?flDocNum=49701-NEA"


def _registry_html_with_status(rows):
    body = []
    for doc_number, bin_number, status in rows:
        body.append(
            "<tr>"
            "<td>10.09.2026 10:00:00</td>"
            f"<td>{doc_number}</td>"
            f"<td>{bin_number}</td>"
            f"<td>ТОО {doc_number}</td>"
            "<td>Заявка на разведку ТПИ</td>"
...
def test_fetch_application_by_number_selects_exact_document_number():
    scraper = _ExactLookupScraper()
    app = scraper.fetch_application_by_number("49701-NEA")

    assert app is not None
    assert app.doc_number == "49701-NEA"
    assert app.status == "Выдана лицензия"
    assert scraper.lookup == (
        1,
        "Заявка на разведку ТПИ",
        (),
        "49701-NEA",
